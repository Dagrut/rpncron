...
namespace RC {
	...
	void Run::runFile(ArgsRc &args) {
		Run me(args);
		me.daemonize();
		me.start();
	}
	
	void Run::runDir(ArgsRc &args) {
		Run me(args);
		me.daemonize();
		me.start();
	}
	
	Run::Run(ArgsRc &args) : args(args) {
	...
	void Run::loadDirectory(const std::string &path) {
		OS::Directory dir;
		OS::File fstats;
		std::string dir_item;
	...
	void Run::loadUsers(const std::string &path) {
		OS::Directory dir;
		OS::File fstats;
		std::string dir_item;
		std::string dir_item_path;
	...
	bool Run::reloadTask(Run::Task *task, time_t now) {
		...
		if(rpn.result_size() == 0) {
			if(task->ce.conf.mode == ConfParser::CONF_MODE_OFFSET) {
				...
				task->exec_time = Run::nextMin(now);
				return(true);
			}
		}
	...
	void Run::initSignals() {
		struct sigaction action;
		action.sa_handler = NULL;
		action.sa_sigaction = Run::processStopNicelyOnSignal;
		...
		action.sa_sigaction = Run::processChildTerminated;
		sigemptyset(&action.sa_mask);
	...
	bool Run::processConfModeBool(Run::Task *t) {
		bool ret;
	...
	bool Run::processConfModeOffset(Run::Task *t) {
		this->updateTaskCounter(t);
		if(t->childs.size() < t->ce.conf.max_proc || t->ce.conf.max_proc == 0) {
			this->executeTask(t);
		}
	...
	void Run::executeTask(Run::Task *t) {
		OS::Processes::ID id;
	...
	void Run::updateTaskCounter(Run::Task *t) {
	...
	void Run::prepareTaskContext(Run::Task *t) {
	...
	void Run::executeTaskSystem(Run::Task *t) {
		for(int i = 0, l = t->ce.cmds_lines.size() ; i < l ; i++) {
			system(t->ce.cmds_lines[i].c_str());
		}
	}
	
	void Run::executeTaskPipe(Run::Task *t) {
	...
	void Run::onExecEnd(Run::Task *t, const std::vector<char> &output, int return_code) {
	...
	void Run::processStopNicelyOnSignal(int sig, siginfo_t *info, void *unused) {
		Run::keep_running = false;
	}
	
	void Run::processChildTerminated(int sig, siginfo_t *info, void *unused) {
		OS::Processes::acceptChildDeath();
	}
	
	time_t Run::curMin(time_t t) {
		t /= 60;
		t *= 60;
		return(t);
	}
	
	time_t Run::nextMin(time_t t) {
		return(Run::curMin(t) + 60);
	}
}
